...
uint8_t SdSpi::receive(uint8_t* buf, size_t n)
{
    ...
    else if (n & 1) // odd amount?
    {
        SPIFIFO.write(0XFF, SPI_CONTINUE);
        *buf++ = SPIFIFO.read();
        --n;
    }
    ...
    if (n < 6)
    {
        const uint8_t *limit = buf + (n - 2);
        while (buf < limit)
        {
            SPIFIFO.write16(0XFFFF, SPI_CONTINUE);
            const uint_fast16_t w = SPIFIFO.read();
            buf[0] = ((w & 0xFF00) >> 8);
            buf[1] = w & 0xFF;
            buf += 2;
        }
        
        SPIFIFO.write16(0XFFFF);
        const uint_fast16_t w = SPIFIFO.read();
        buf[0] = ((w & 0xFF00) >> 8);
        buf[1] = w & 0xFF;
    }
    else
    {
        ...
        const uint8_t *limit = buf + (n - 2) - (2*3);
        while (buf < limit)
        {
            SPIFIFO.write16(0XFFFF, SPI_CONTINUE);
            const uint_fast16_t w = SPIFIFO.read();
    ...
    if (n > 4) // use FIFO?
    {
        ...
        for (size_t i = 3; i < (n-1); ++i) {
            SPIFIFO.write(0xFF, SPI_CONTINUE);
            buf[i-3] = SPIFIFO.read();
        }
        
        SPIFIFO.write(0xFF);
        buf[n-3-1] = SPIFIFO.read();
        
        buf[n-3] = SPIFIFO.read();
        buf[n-2] = SPIFIFO.read();
        buf[n-1] = SPIFIFO.read();
    }
    else
    {
        for (size_t i = 0; i < (n-1); ++i) {
            SPIFIFO.write(buf[i], SPI_CONTINUE);
            SPIFIFO.read();
        }
        
        SPIFIFO.write(buf[n-1]);
        SPIFIFO.read();
    }
#endif
...
void SdSpi::send(const uint8_t* buf, size_t n)
{
    ...
    if (n & 1) // odd amount?
    {
        SPIFIFO.write(*buf++, SPI_CONTINUE);
        SPIFIFO.read();
        --n;
    }
    
    const uint_fast8_t nf = (n/2) < 3 ? (n/2) : 3;
    const uint8_t *limit = buf + (n - 2);
    
    for (size_t i=0; i<nf; ++i)
    {
        SPIFIFO.write16((buf[0] << 8) | buf[1], SPI_CONTINUE); // Push in 2 bytes, reverse order
        buf += 2;
    }

    while (buf < limit)
    {
        SPIFIFO.write16((buf[0] << 8) | buf[1], SPI_CONTINUE);
        buf += 2;
        SPIFIFO.read();
    }
    
    SPIFIFO.write16((buf[0] << 8) | buf[1]);
    ...
    if (n > 4) // use FIFO?
    {
        SPIFIFO.write(buf[0], SPI_CONTINUE);
        SPIFIFO.write(buf[1], SPI_CONTINUE);
        SPIFIFO.write(buf[2], SPI_CONTINUE);
